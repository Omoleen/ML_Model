...
# import the dataset
df = pd.read_csv('sarcoma_mskcc_2022_clinical_data.tsv', sep='\t')
# print(data)
...
# print(df.columns)
df.drop(['Patient_ID', 'Sample_ID', 'Cancer_Type_Detailed',
         'Sample_Type', 'Ethnicity_Category', 'Race_Category',
         'Detailed_Primary_Site','FACETs_Review', 'Gene_Panel',
         'FACETS'], axis=1, inplace=True)
#
...
num_cols = ['Mutation_Count', 'Fraction_Genome_Altered', 'FGA_Reported', 'Fraction_CNA',
            'Impact_TMB_Score', 'MSI_Score']
df_ready[num_cols] = scaler.fit_transform(df_ready[num_cols])
# replace categorical data
df_ready['Cancer_Type'].replace(['Soft Tissue Sarcoma', 'Gastrointestinal Stromal Tumor', 'Bone Cancer', 'Uterine Sarcoma', 'Nerve Sheath Tumor'],
                        [0, 1, 2, 3, 4], inplace=True)
df_ready["Overall_Survival_Status"].replace(['1:DECEASED', '0:LIVING'],
                      [0, 1], inplace=True)
df_ready["FACETs_WGD"].replace([False, True], [0, 1], inplace=True)
df_ready["Sex"].replace(['Female', 'Male'], [0, 1], inplace=True)
# df_ready["Age_at_Which_Sequencing_was_Reported_(Days)"].replace(['>90'], [''], inplace=True)
# # print(df_ready)
# df_ready =df_ready.dropna()
df_ready = df_ready[df_ready["Age_at_Which_Sequencing_was_Reported_(Days)"].str.contains(">90")==False]
print(df_ready.groupby("Overall_Survival_Status").size())  # print number of dead or alive

Dead = df_ready[df_ready["Overall_Survival_Status"] == 0]
Alive = df_ready[df_ready["Overall_Survival_Status"] == 1]
# print(Dead.shape)
# print(Alive.shape)
...
X_train_smote = df_ready.drop(["Overall_Survival_Status"], axis=1)
Y_train_smote = df_ready["Overall_Survival_Status"]
...
# Saving model
dump(rf, 'Origin/sacroma_randomforest.joblib')
dump(grid_search, 'Origin/sacroma_crossvalid_gridsearch.joblib')
